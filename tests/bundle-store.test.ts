...
describe('BundleStore (MemoryStore)', () => {
  ...
  it('save and load round-trip', async () => {
    await store.save('proj_test12345678', 'en', {
      json: '{"format_version":1,"strings":{}}',
      etag: '"rev:42"',
    })

    const loaded = await store.load('proj_test12345678', 'en')
  ...
  it('returns null when empty', async () => {
    const loaded = await store.load('proj_nonexistent', 'en')
  ...
  it('isolates per locale', async () => {
    ...
    await store.save('proj_test12345678', 'fr', {
      json: '{"locale":"fr"}',
      etag: '"fr:1"',
    })

    const enLoaded = await store.load('proj_test12345678', 'en')
    const frLoaded = await store.load('proj_test12345678', 'fr')
  ...
  it('delete removes cache', async () => {
    ...
    expect(await store.load('proj_test12345678', 'en')).not.toBeNull()

    await store.delete('proj_test12345678', 'en')
    expect(await store.load('proj_test12345678', 'en')).toBeNull()
  })
  ...
  it('isolates per project', async () => {
    await store.save('proj_aaa', 'en', {
      json: '{"project":"aaa"}',
      etag: null,
    })
    await store.save('proj_bbb', 'en', {
      json: '{"project":"bbb"}',
      etag: null,
    })

    const aaa = await store.load('proj_aaa', 'en')
    const bbb = await store.load('proj_bbb', 'en')
